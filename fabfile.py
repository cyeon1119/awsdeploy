...
# 원격 pc에서 자동으로 서버를 세팅하는 자동화 모듈이다.
...
# 최초 배포, 업그레이드 배포
# 업그레이드 배포시 명령
# 절차상으론 코드수정 -> 로컬테스트완료 -> git 커밋, push -> fab deploy
# -> 상용서버에 반영완료 (유리보수및 서비스 관리 차원)
# $> fab deploy
def deploy():
    ...
    # 가상환경을 업데이트
    _update_virtualenv()
    # 서버가 가동될때 홈디렉터리, 로그파일의 위치등을 설정
    # *.conf가 수정 및 생성된다
    # wsgi.py 엔트리포인트가 설정되어있다.
    _make_virtualhost()
...
def _get_latest_source():
    # .git 이 있으면 저장소에가서 추가된 내용을 패치해온다
    # 패치 -> 가져온다
    if exists(project_folder + '/.git'):
        run('cd %s && git fetch' % (project_folder,))
    # .git가 해당프로젝트에 없으면 저장소에 가서 프로젝트 내용을 가져온다
    else:
...
def _update_virtualenv():
    ...
    # 필요한 패키지를 설치하시오
    # 핵심 -> pip install -r %s/requirements.txt
    ...
    # 해당가상환경안에, flask, scikit-learn 이 2개가 설치될것이다
